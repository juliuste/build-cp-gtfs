...
const stopoverRequest = (station, dateTime) => {
	...
	return cp.stopovers(station, { when: dateTime.toJSDate() }).catch(error => {
		console.error('error', error.statusCode, get(error.body, 'description'))
		return []
	})
}
...
const build = async (startDate, endDate) => {
	...
	const gtfs = {
		...
		routes: toStream([
			...
			...lines.map(l => {
				const shortName = l.productCode ? [l.productCode, l.name].join(' ') : l.name
				const longName = l.product ? [l.product, l.name].join(' ') : ''
				return [l.id, 'cp', shortName, longName, '', 2, '', '', '', '']
			}) // @todo: type
		]),
		trips: stream(),
		stop_times: stream(),
